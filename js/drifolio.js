...
$(function() {
	...
	var button = '.dribbble-follow-button',
		label = $(button).text(),
		username = $('a'+button).attr('href').toLowerCase().replace('', ''),
	...
	$.getJSON('http://api.dribbble.com/players/'+username+'?callback=?', function(data) {
        ...
        .parent().html('<a class="label" href="'+username+'" target="_blank"><i></i>'+label+'</a><a class="count" href="'+username+'/followers" target="_blank"><i></i><u></u>'+data.followers_count+' followers</a>');
